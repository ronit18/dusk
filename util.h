...
static const uint64_t
    ...
    BarActiveGroupBorderColor = 0x20, // use border color of active group,
                                      // otherwise title scheme is used
    BarMasterGroupBorderColor = 0x40, // use border color of master group,
                                      // otherwise title scheme is used
    ColorEmoji = 0x80,
    Status2DNoAlpha =
        0x100,         // option to not use alpha when drawing status2d status
    Systray = 0x200,   // enables systray
    BarBorder = 0x400, // draw a border around the bar
    NoBorders = 0x800, // as per the noborder patch, show no border when only
                       // one client in tiled mode
    Warp = 0x1000,     // warp patch
    FocusedOnTop = 0x2000,
    DecorationHints = 0x4000, // used by setfullscreen, prevents state change
    FocusOnNetActive = 0x8000, AllowNoModifierButtons = 0x10000,
    CenterSizeHintsClients = 0x20000, // center tiled clients subject to size
                                      // hints within their tiled area
    ResizeHints = 0x40000, // if enabled then dusk will respect size hints in
                           // tiled resizals
    SortScreens = 0x80000, // only applies on startup
    ViewOnWs =
        0x100000, // follow a window to the workspace it is being moved to
    ...
    AltWorkspaceIcons =
        0x1000000, // show the workspace name instead of the icons
    GreedyMonitor =
        0x2000000, // when viewing a workspace the monitor is greedy and gives
                   // nothing in return (i.e. disables swap of workspaces)
    SmartLayoutConvertion =
        0x4000000, // when moving a workspace from one monitor to another,
                   // automatically adjust layout based on monitor orientation
                   // (i.e. vertical vs horizontal)
    AutoHideScratchpads = 0x8000000, // automatically hide open scratchpads when
                                     // moving to another workspace
    RioDrawIncludeBorders =
        0x10000000, // indicates whether the area drawn using slop includes the
                    // window borders
    RioDrawSpawnAsync =
        0x20000000, // indicates whether to spawn the application alongside or
                    // after drawing area using slop
    BarPadding = 0x40000000, // enables bar padding
    RestrictFocusstackToMonitor =
        0x80000000, // option to restrict focusstack to only operate within the
                    // monitor
    AutoReduceNmaster =
        0x100000000, // automatically reduce nmaster if there are multiple
                     // master clients and one is killed
    WinTitleIcons =
        0x200000000, // adds application icons to window titles in the bar
    WorkspacePreview = 0x400000000, // adds preview images when hovering
                                    // workspace icons in the bar
    SpawnCwd = 0x800000000, // spawn applications in the currently selected
                            // client's working directory
    SystrayNoAlpha =
        0x1000000000, // option to not use alpha (transparency) for the systray
    WorkspaceLabels = 0x2000000000, // adds the class of the master client next
                                    // to the workspace icon
    FlexWinBorders =
        0x4000000000, // option to use the SchemeFlex* colour schemes, if
                      // disabled then SchemeTitle* is used instead
    ...
    Debug = 0x100000000000000, // same name and value used for client flags, see
                               // flags.h
